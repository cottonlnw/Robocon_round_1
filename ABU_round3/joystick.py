...
class Ps5reader:
    ...
    def read_ps5(self):
        ...
        try:
            ...
            print(f'Controller lost: {e}. Reconnecting...')
            self.controller = None
            return None  # ❗แนะนำให้ return None ตอน error
    
    def scale_axes(self, raw_axes):
        NORMALIZATION_FACTOR = 0.95  # แก้ไขค่าคลาดเคลื่อนของจอย
        
        scaled_axes = [round(a / NORMALIZATION_FACTOR * 255) for a in raw_axes]

        # รีแมป L2 (axis 2) ให้เป็น 0–100
        if len(raw_axes) > 2:
